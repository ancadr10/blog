...
export const registerController = async (req: Request, res: Response): Promise<any> => {
    ...
    await addToken(token, 'activation', user.id!); //save token in db
...
////email////
export const confirmEmailController = async (req: Request, res: Response): Promise<any> => {
    ...
    res.redirect(process.env.FRONTEND_BASE_URL + '/auth/login');
}

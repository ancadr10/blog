...
export class AuthService {
  ...
  login({ email, password }: { email: string, password: string }) {
    let ob$ = this.http.post<Session>(`${environment.BACKEND_API_URL}/${this.baseUrl}/login`, { email, password })
      .pipe(
        share()
      );
  ...
  register({ name, email, password }: { name: string, email: string, password: string }) {
    return this.http.post(`${environment.BACKEND_API_URL}/${this.baseUrl}/register`, { name, email, password });
  }
